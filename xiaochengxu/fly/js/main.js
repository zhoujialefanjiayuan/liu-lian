...
var planscore=document.getElementById("planscore");
    //获取用户id
var user=document.getElementById("label").getAttribute('user');
    //初始化分数
var scores=0;
...
移动行为
    ...
    this.planmove=function(){
        if(scores<=50000){
            ...
            this.imagenode.style.top=this.imagenode.offsetTop+this.plansudu+2.5+"px";
        }
    }
    this.init=function(){
...
function ourplan(X,Y){
    var imagesrc="/static/image/我的飞机.gif";
    plan.call(this,1,X,Y,66,80,0,660,0,"/static/image/本方飞机爆炸.gif",imagesrc);
...
function start(){
    ...
    if(mark==20){
        ...
        //中飞机
        if(mark1%5==0){
            enemys.push(new enemy(6,25,274,46,60,5000,360,random(1,3),"/static/image/中飞机爆炸.gif","/static/image/enemy3_fly_1.png"));
        }
        //大飞机
        if(mark1==20){
            enemys.push(new enemy(12,57,210,110,164,30000,540,1,"/static/image/大飞机爆炸.gif","/static/image/enemy2_fly_1.png"));
            mark1=0;
        }
        //小飞机
        else{
            enemys.push(new enemy(1,19,286,34,24,1000,360,random(1,4),"/static/image/小飞机爆炸.gif","/static/image/enemy1_fly_1.png"));
        }
        mark=0;
    }
            ...
            if(enemys[j].planisdie==false){
                if(enemys[j].imagenode.offsetLeft+enemys[j].plansizeX>=selfplan.imagenode.offsetLeft&&enemys[j].imagenode.offsetLeft<=selfplan.imagenode.offsetLeft+selfplan.plansizeX){
                  if(enemys[j].imagenode.offsetTop+enemys[j].plansizeY>=selfplan.imagenode.offsetTop+40&&enemys[j].imagenode.offsetTop<=selfplan.imagenode.offsetTop-20+selfplan.plansizeY){
                      //碰撞本方飞机，游戏结束，统计分数
                      selfplan.imagenode.src="/static/image/本方飞机爆炸.gif";
...
// 提交分数
function putscore(){
    // var up_score = scores.toString()

    $.ajax({
    url: 'http://101.132.47.14:8000/games/putscore/?score='+scores +'&user='+user,
        type:"GET",
    success: function(e){
        if (e.code==1){
            location.reload(true);
